...
def get_spark_session():
    ...
    try:
        ...
        if not IS_DATABRICKS:
            print("ℹ️ Running locally without Databricks Connect. Skipping local Spark initialization to avoid hanging.")
            return None
...
if IS_DATABRICKS:
    DBFS_DATA_PATH = "/dbfs/mnt/data"
else:
    # Local path for Windows/Local development
    DBFS_DATA_PATH = os.path.join(os.getcwd(), "data")
    os.makedirs(DBFS_DATA_PATH, exist_ok=True)
    print(f"📁 Local data path: {DBFS_DATA_PATH}")
...
print(f"Train: {len(train_df)} ({len(train_df)/max(1, len(df_features))*100:.1f}%)")
print(f"Val:   {len(val_df)} ({len(val_df)/max(1, len(df_features))*100:.1f}%)")
print(f"Test:  {len(test_df)} ({len(test_df)/max(1, len(df_features))*100:.1f}%)")
...
for split_name, split_df in [("train", train_df), ("val", val_df), ("test", test_df)]:
    if spark is not None:
        ...
        print(f"ℹ️ Spark session not available. Saving {split_name} set to local CSV.")
        split_path = os.path.join(DBFS_DATA_PATH, f"stock_data_{split_name}.csv")
        split_df.to_csv(split_path, index=False)
        print(f"✓ Saved to local CSV: {split_path}")
